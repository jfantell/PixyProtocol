use cosmwasm_std::{to_binary, StdResult, CosmosMsg, WasmMsg, Coin, DepsMut, Deps, Uint128};
...
pub fn deposit_stable_msg(deps: DepsMut, amount: Uint128, denom: String) -> StdResult<CosmosMsg> {
    ...
    Ok(CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: cfg.anchor_money_market_address,
...
pub fn redeem_stable_msg(deps: &DepsMut, amount: Uint128, denom: String) -> StdResult<CosmosMsg> {
    let cfg = CONFIG.load(deps.storage)?;
    let anchor_earn_contract_address = cfg.anchor_money_market_address;
    let a_ust_address = cfg.a_ust_address;
    Ok(CosmosMsg::Wasm(WasmMsg::Execute {
